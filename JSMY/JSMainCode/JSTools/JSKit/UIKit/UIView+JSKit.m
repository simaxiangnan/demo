...
///* 6.设置圆角半径 */
//- (void)js_setCornerRadius:(CGFloat)radius
//{
...
- (void)setManager:(JSKitManager *)manager
{
    objc_setAssociatedObject(self, @selector(jskit_manager), manager, OBJC_ASSOCIATION_RETAIN);
}

- (JSKitManager *)getManager
{
    return objc_getAssociatedObject(self, _cmd);
}

- (JSKitManager *)jskit_manager
{
    JSKitManager *manager = [self getManager];
    if (!manager)
    {
        manager = [JSKitManager new];
    }
    manager.main = self;
    
    if ([self isKindOfClass:[UILabel class]])
    {
        manager.js_label = (UILabel *)self;
    }
...
//
//    dispatch_async(dispatch_get_glojsl_queue(QOS_CLASS_USER_INITIATED, 0), ^{
//
